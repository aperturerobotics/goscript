...
export async function main(): globalThis.Promise<void> {
	...
	let zeroInt = (ZeroValue({T: { type: { kind: $.TypeKind.Basic, name: "int", typeName: "main.IntVal" }, zero: () => 0, methods: {String: (receiver: any, ...args: any[]) => (IntVal_String as any)($.pointerValue(receiver), ...args)} }}) as IntVal)
	$.println("ZeroValue[IntVal]:", IntVal_String(zeroInt))

	// Test 2: Zero value of StringVal should be ""
	let zeroStr = (ZeroValue({T: { type: { kind: $.TypeKind.Basic, name: "string", typeName: "main.StringVal" }, zero: () => "", methods: {String: (receiver: any, ...args: any[]) => (StringVal_String as any)($.pointerValue(receiver), ...args)} }}) as StringVal)
	$.println("ZeroValue[StringVal] len:", $.len(StringVal_String(zeroStr)))

	// Test 3: CallString on zero value
	$.println("CallString on zero IntVal:", CallString({T: { type: { kind: $.TypeKind.Basic, name: "int", typeName: "main.IntVal" }, zero: () => 0, methods: {String: (receiver: any, ...args: any[]) => (IntVal_String as any)($.pointerValue(receiver), ...args)} }}, zeroInt))
	$.println("CallString on zero StringVal len:", $.len(CallString({T: { type: { kind: $.TypeKind.Basic, name: "string", typeName: "main.StringVal" }, zero: () => "", methods: {String: (receiver: any, ...args: any[]) => (StringVal_String as any)($.pointerValue(receiver), ...args)} }}, zeroStr)))

	// Test 4: Sum returns zero value
	let sumInt = (Sum({T: { type: { kind: $.TypeKind.Basic, name: "int", typeName: "main.IntVal" }, zero: () => 0, methods: {String: (receiver: any, ...args: any[]) => (IntVal_String as any)($.pointerValue(receiver), ...args)} }}, null) as IntVal)
	$.println("Sum[IntVal]():", IntVal_String(sumInt))

	let sumStr = (Sum({T: { type: { kind: $.TypeKind.Basic, name: "string", typeName: "main.StringVal" }, zero: () => "", methods: {String: (receiver: any, ...args: any[]) => (StringVal_String as any)($.pointerValue(receiver), ...args)} }}, null) as StringVal)
