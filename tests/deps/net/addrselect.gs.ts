...
export class ipAttr {
	...
	constructor(init?: Partial<{Scope?: scope, Precedence?: number, Label?: number}>) {
		this._fields = {
			Scope: $.varRef(init?.Scope ?? 0),
			Precedence: $.varRef(init?.Precedence ?? 0),
			Label: $.varRef(init?.Label ?? 0)
		}
	}
