...
export class Error {
	...
	constructor(init?: Partial<{msg?: string, timeout?: boolean, temporary?: boolean}>) {
		this._fields = {
			msg: $.varRef(init?.msg ?? ""),
			timeout: $.varRef(init?.timeout ?? false),
			temporary: $.varRef(init?.temporary ?? false)
		}
	}
	...
	static __typeInfo = $.registerStructType(
		...
		Error,
		{"msg": { kind: $.TypeKind.Basic, name: "string" }, "timeout": { kind: $.TypeKind.Basic, name: "bool" }, "temporary": { kind: $.TypeKind.Basic, name: "bool" }}
	)
}
