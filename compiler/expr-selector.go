...
func (c *GoToTSCompiler) isReceiverAlias(expr ast.Expr) bool {
	...
	// Simple approach: Only treat variables as receiver aliases if we're confident
	// they are NOT function parameters.
	//
	// The safest approach is to only identify receiver aliases in specific contexts
	// where we know they're generated by the compiler as "const x = this" patterns.
	//
	// For now, be very conservative and only treat variables as receiver aliases
	// when we have strong evidence they are such aliases.
