...
export function SortImports(fset: token.FileSet | null, f: File | null): void {
	...
	for (let _i = 0; _i < $.len(f!.Decls); _i++) {
		const d = f!.Decls![_i]
		{
			let _gs_ta_val_acd9: GenDecl | null
			let _gs_ta_ok_acd9: boolean
			({ value: _gs_ta_val_acd9, ok: _gs_ta_ok_acd9 } = $.typeAssert<GenDecl | null>(d, {kind: $.TypeKind.Pointer, elemType: 'GenDecl'}))
			let d = _gs_ta_val_acd9
			let ok = _gs_ta_ok_acd9
...
export function sortSpecs(fset: token.FileSet | null, f: File | null, specs: $.Slice<Spec>): $.Slice<Spec> {
	...
	for (let i = 0; i < $.len(specs); i++) {
		const s = specs![i]
		{
			let _gs_ta_8657 = $.mustTypeAssert<ImportSpec | null>(s, {kind: $.TypeKind.Pointer, elemType: 'ImportSpec'})
			let s = _gs_ta_8657
			if (s!.Name != null) {
